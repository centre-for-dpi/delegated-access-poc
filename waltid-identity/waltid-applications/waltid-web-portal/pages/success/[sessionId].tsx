...
export default function Success() {
  ...
  const [overallResult, setOverallResult] = useState<boolean>(true);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        setOverallResult(response.data.verificationResult === true);
  ...
  return (
    <div className="h-screen flex justify-center items-center bg-gray-50">
      ...
      <div className="relative w-full h-full sm:h-auto sm:w-10/12 md:w-8/12 lg:w-6/12 text-center shadow-2xl rounded-lg pt-8 pb-8 px-10 bg-white">
        <h1 className={`text-3xl text-center font-bold mb-10 ${overallResult ? 'text-gray-900' : 'text-red-600'}`}>
          {overallResult ? 'Presented Credentials' : 'Verification Failed'}
        </h1>
          ...
          <div className="group h-[225px] w-[400px] [perspective:1000px]">
            <div className="relative h-full w-full rounded-xl shadow-xl transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
              <div className="absolute inset-0">
                <div className={`flex h-full w-full flex-col drop-shadow-sm rounded-xl py-7 px-8 text-gray-100 cursor-pointer overflow-hidden bg-gradient-to-r z-[-2] ${
                  policyResults[index + 1]?.policyResults.some(p => !p.is_success)
        ...
        <div className="mt-10 px-12">
          <div className={`flex flex-row items-center justify-center mb-5 ${
            policyResults[index + 1]?.policyResults.some(p => !p.is_success) ? 'text-red-500' : 'text-gray-500'
          }`}>
            {policyResults[index + 1]?.policyResults.length
              ? policyResults[index + 1]?.policyResults.some(p => !p.is_success)
                ? 'Verification failed for the following policies:'
