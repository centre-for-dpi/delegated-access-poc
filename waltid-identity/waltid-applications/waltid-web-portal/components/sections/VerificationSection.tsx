...
export default function VerificationSection() {
  ...
  const [credentialStatusPolicy, setCredentialStatusPolicy] = useState<boolean>(true);
  ...
  function handleVerify() {
    ...
    if (credentialStatusPolicy) {
      vps.push('credential-status');
    }
    if (webhookPolicy) {
      if (webhook.length == 0) {
    ...
    <>
      ...
      <div className="flex flex-row justify-start mt-8">
        <div className="flex flex-col gap-3 w-full">
          ...
          />
          <PolicyListItem
            name="Credential Status Policy"
            value={credentialStatusPolicy}
            onChange={setCredentialStatusPolicy}
          />
