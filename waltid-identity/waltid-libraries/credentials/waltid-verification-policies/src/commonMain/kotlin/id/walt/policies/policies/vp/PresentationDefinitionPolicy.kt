            ...
            if (hasSameSubjectConstraint) {
                inputDescriptor.constraints.fields?.filter { it.id != null }?.forEach { field ->
                    ...
                    matchedCredentials.forEach { cred ->
                        // Resolve the value using the field's actual JSON path(s),
                        // rather than always extracting the top-level credential subject.
                        val resolvedValue = field.path.firstNotNullOfOrNull { path ->
                            cred.resolveOrNull(JsonPath.compile(path))
                        }?.jsonPrimitive?.contentOrNull

                        if (resolvedValue != null) {
                            subjectDidsPerFieldId.getOrPut(fieldId) { mutableSetOf() }.add(resolvedValue)
                        } else {
                            log.debug { "Could not resolve field path ${field.path} for field $fieldId in descriptor ${inputDescriptor.id}" }
                        }
                    }
                }
            }
